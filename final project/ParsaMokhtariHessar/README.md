...
<div dir="rtl">
سیستم طراحی شده توسط جناب شکری برای خواندن کاراکتر ها زیاد قابل مقیاس دادن نبود برای همین سیستمی با استفاده از حلقه برای خواندن کاراکتر ها طراحی شد که مقیاس پذیری آن بیشتر است . بگفته برنامه نویسان حرفه ای هر کاه شما در کد نویسی خود کپی پیست زیاد انجام می دهید بهتر است روش خود را عوض کنید.
</div>



<div dir="rtl">
در این جا این خط کد کاراکتر ها را می گیرید و در یک آرایه بصورت خود کار ذخیره می کند/
</div>
...
```
<div dir="rtl">
برای جلوگیری از مشکلات مربوط به نوع داده های مختلف کد زیر کاراکتر ها را به اینتیجر هشت بیتی تبدیل می کند.
</div>
...
<div dir="rtl">
 از آنجایی که طول و عرض هر کاراکتر به ازای هر حرف گرفته می شود. ما برای متناسب تعیین نمودن سایز اینیشالایز کپچا عرض را ضرب در تعداد تعیین شده حروف کپچا می نماییم 
</div>
...
<div dir="rtl">
به تعداد حروفی که باید در کپچای حال حاضر باشد بایستی کارکتر ها را سوار تصویر کپچای از قبل درست شده (در خطکد قبلی) بکنیم
</div>
...
```
        temp = Rotation(alphabet(:,:,captcha_alphabet(i)),randi([0 360]));
```

<div dir="rtl">
همچنین در هر سلول خطوط رندم درست می کنیم برای مثال در کد زیر 6 خط رندم درست شده است که بعد از ریسایز کردن به خطوط نقطه چین زخیم تبدیل می شوند.
</div>

```
        temp = RandLine(temp,2,10,10,500,6);
```
...
```
    captcha_image = RandLine(captcha_image,2,10,10,1500,50);
```

<div dir="rtl">
برای حتی سخت تر شدن کپچا شکل های رندم را نیز به آن می افزاییم.تابع این شکل ها با استفاده از تمارین قبلی ساخته شده و شامل دایره، لوزی،مربع،مستطیل و مثلث می باشد .
</div>

```
    captcha_image = RandomShapes(captcha_image,20,15);
```
...
<div dir="rtl">
برخی از String ها در دو خانه آرایه دخیره شده اند باید آنها را به یک String تبدیل نمود .برای این کار از حلقه زیر و تابعstrcat استفاده می کنیم.
</div>
...
<div dir="rtl">
سپس استریگ پردازش شده را با نام بدون اکستنشن عکس مقایسه می کنیم اگر برابر بود یک واحد به متغییر درست و اگر نابرابر یا خالی بود یک واحد به متغییر نا درست اضافه می شود.
</div>
...
<div dir="rtl">
سپس تعداد درست ها، نادرست ها و درصد درست ها از نعداد کل را محاسبه کرده و نمایش می دهیم. 
</div>    
...
<div dir="rtl">
برای تست درست عمل کردن این تستر کپچا 15 عدد کپچا بدون نویز تولید شده که در پوشهOutPutClean گزاشته شده و اعمال کردن آن دقت 80% برای OCRTest می دهد. 
</div>
...
<div dir="rtl">
این کد با استفاده از معادله خط کل صفحه را پیمایش کرده و برای ما یک خط با ضرایبxوy و عرض از مبداb رسم می کند.
</div>
...
<div dir="rtl">
این تابع رندم سازی های بسیار ی را نیاز دارد . از خود شکل گرفته تا مرکز آن ، اندازه آن و حتی زخیمی آن .
</div>


<div dir="rtl">
این تابع تصویر  و تعداد شکل های در خواستی و ماکزیمم شعاع را ضلع آنها را می گیرد
</div>
...
<div dir="rtl">
شعاع را ضلع بصورت رندم با توجه به شعاع بیشینه دریافت شده توسط فرا خواننده تولید شده و در آرایه شعاع ها ریخته می شود.
</div>
...
<div dir="rtl">
آرایه ای برای جا دادن مراکز شکل ها شاخته می شود.
</div>
...
<div dir="rtl">
مراکز شکل ها بصورت رندم با توجه به طول و عرض تصویر دریافتی در آرایه از قبل درسته شده ریخته می شود.
</div>
...
<div dir="rtl">
دستور Switch در یک حلقه شرع به فعالیت کرده و یکی یکی شکل ها را رسم می کند.
</div>
